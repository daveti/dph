...
sock_recv_guts(PySocketSockObject *s, char* cbuf, int len, int flags)
{
    ...
    Py_BEGIN_ALLOW_THREADS
    // daveti: hack
    printf("daveti: socketmodule interval [%f]\n", interval);
    timeout = internal_select_ex(s, 0, interval);
    if (!timeout) {
	printf("daveti: socketmodule recv\n");
        outlen = recv(s->sock_fd, cbuf, len, flags);
	printf("daveti: recv outlen [%ld]\n", outlen);
    }
    printf("daveti: socketmodule recv done\n");
    Py_END_ALLOW_THREADS

    if (timeout == 1) {
	printf("daveti: socketmodule timeout\n");
        PyErr_SetString(socket_timeout, "timed out");
        return -1;
    }
    printf("daveti: socketmodule loop again\n");
    END_SELECT_LOOP(s)
    printf("daveti: socketmodule loop end\n");
    if (outlen < 0) {
        /* Note: the call to errorhandler() ALWAYS indirectly returned
           NULL, so ignore its return value */
	printf("daveti: outlen [%ld], rtn [-1]\n", outlen);
        s->errorhandler();
        return -1;
    }
#else
...
sock_recv(PySocketSockObject *s, PyObject *args)
{
    ...
    /* Call the guts */
    printf("daveti: start sock_recv_guts\n");
    outlen = sock_recv_guts(s, PyString_AS_STRING(buf), recvlen, flags);
    printf("daveti: done sock_recv_guts, outlen [%ld], recvlen [%d]\n", outlen, recvlen);
    if (outlen < 0) {
