...
fill_free_list(void)
{
    ...
    //daveti: convert the objects[N_INTOBJECTS] into single link list
    //ob_type within the PyObject is hacked to be used as the 'next' pointer...
    //which means the int obj from big int pool does NOT have ob_type? - NO!
    p = &((PyIntBlock *)p)->objects[0];
    q = p + N_INTOBJECTS;
    while (--q > p)
        Py_TYPE(q) = (struct _typeobject *)(q-1);
    //daveti: This NULL is used to nil free_list!
    Py_TYPE(q) = NULL;
    return p + N_INTOBJECTS - 1;
}

//daveti: small int pool!
...
int_dealloc(PyIntObject *v)
{
	...
	//daveti: smart!
	//Any free'd PyIntObject from big int pool will be
	//linked into the free_list again!
...
/* ARGSUSED */
//daveti: hack to print int pool usage
     ...
     /* flags -- not used but required by interface */
{
	...
	printf("daveti: address @[%p]\n", v);

	// Retrive the first 10 PyIntObj in use!
	// It should be the small int pool: [-5, 4]	
	for (i = 0; i < 10; ++i, --intObjectPtr) {
		values[i] = intObjectPtr->ob_ival;
		refcounts[i] = intObjectPtr->ob_refcnt;
	}
	printf("daveti: values: ");
	for (i = 0; i < 10; ++i)
		printf("%d\t", values[i]);
	printf("\n");
	printf("daveti: refcnt: ");
	for (i = 0; i < 10; ++i)
		printf("%d\t", refcounts[i]);
	printf("\n");

	printf("daveti: block_list count [%d]\n"
		"daveti: free_list @[%p]\n",
		count, free_list);
	//daveti: hack done
