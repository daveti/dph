...
static PyObject *PySSL_SSLread(PySSLObject *self, PyObject *args)
{
	...
	//daveti: socket state is OK
	printf("daveti: no match for count [0], sockstate [%d]\n", sockstate);
    }
    do {
        PySSL_BEGIN_ALLOW_THREADS
        count = SSL_read(self->ssl, PyString_AsString(buf), len);
	//daveti: as long as count > 0, SSL_get_error will NOT return any error!
	...
	//daveti:
	printf("daveti: count [%d] err [%d] after SSL_read()\n", count, err);
	    ...
	    //daveti: reset socket state here
            sockstate = SOCKET_OPERATION_OK;
	    printf("daveti: prev-sockstate [%d], next-sockstate [%d]\n",
	    ...
	    // daveti:
	    printf("daveti: break do - sockstate [SOCKET_IS_NONBLOCKING]\n");
            break;
        }
    } while (err == SSL_ERROR_WANT_READ || err == SSL_ERROR_WANT_WRITE);
    if (count <= 0) {
	//daveti:
	printf("daveti: count <= 0\n");
        Py_DECREF(buf);
        return PySSL_SetError(self, count, __FILE__, __LINE__);
    }
    if (count != len) {
	printf("daveti: count [%d], len [%d]\n", count, len);
